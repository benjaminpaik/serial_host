...
String generateConfigFile(ConfigData configData) {
  ...
  inputText += "=> baud rate: ${configData.baudRate.toString()}$newline";
  inputText += "=> message period: ${configData.commPeriod}$newline";
  inputText += "=> network ID: ${configData.networkId.toString()}$newline";
  inputText +=
...
ConfigData parseConfigFile(String configText) {
  ...
  for (var line in lines) {
    ...
    if (line.contains("=>")) {
      ...
      if (line.contains("baud rate")) {
        try {
          configData.baudRate = int.parse(fieldText);
        } catch (e) {
          throw const FormatException("invalid baud rate");
        }
      } else if (line.contains("message period")) {
        try {
          configData.commPeriod = int.parse(fieldText);
        } catch (e) {
          throw const FormatException("invalid message period");
        }
      } else if (line.contains("network ID")) {
        try {
          configData.networkId = int.parse(fieldText);
        } catch (e) {
          throw const FormatException("invalid network ID");
        }
      } else if (line.contains("command range")) {
        try {
...
Future<Object> convertDataFile(
  ...
  final firstRow = dataLines.first;
  final firstLine = firstRow
      .substring(1, firstRow.length - 1)
      .split(',')
      .map((e) => int.parse(e))
      .toList();
  final requestedStates = (firstLine[SerialParse.stateIndex0] << 8) |
      firstLine[SerialParse.stateIndex1];
  bool parseStatus = (configData.status.numFields > 0 &&
      (requestedStates & (1 << configData.status.stateIndex) != 0));

  writer.write(parseDataHeaders(configData, requestedStates));
  await writer.flush();
  writer.writeAll(
      DataRowIterable(configData, requestedStates, parseStatus, dataLines));
  await writer.close();
  if (!saveByteFile) {
...
String parseDataHeaders(ConfigData configData, int requestedStates) {
  String headerData = "time(ms), network ID, mode, stateback, ";
  for (int i = 0; i < configData.telemetry.length; i++) {
...
int parseTimeDiff(ConfigData configData, int timeCurrent, int timePrevious) {
  int timeDiff = 0;
  if (configData.commPeriod <= SerialParse.timestampHostThreshold) {
  ...
  // we should assume the comm period is the message rate
  else {
    timeDiff = configData.commPeriod;
  }
  return timeDiff;
}
...
String parseDataRow(ConfigData configData, int requestedStates,
  ...
  int stateBack =
      (bytes[SerialParse.stateIndex0] << 8) | bytes[SerialParse.stateIndex1];
  int stateIndex = 0;
  ...
  // add the timestamp, network ID, command mode, and stateback bitfield
  rowText =
      "$timeStamp, ${bytes[SerialParse.networkIndex]}, ${bytes[SerialParse.commandModeIndex]}, $stateBack, ";
