...
import 'package:serial_host/widgets/oscilloscope_widget.dart';
...
class HostDataModel extends ChangeNotifier {
  ...
  bool setNetworkId(int? id) {
    ...
    if (id != null) {
      try {
        configData.networkId = id;
        SerialParse.setNetworkId(serial, configData.networkId);
        serial.sendPacket();
        notifyListeners();
        return true;
      } catch (e) {
        _userMessage = Message.error.networkIdRange;
      }
    } else {
      _userMessage = Message.error.networkIdText;
    }
    return false;
  }
  ...
  set baudRate(int value) {
    configData.baudRate = value;
    notifyListeners();
  }

  set commPeriod(int value) {
    if (value > 0) {
      configData.commPeriod = value;
      serial.commPeriod = configData.commPeriod;
      notifyListeners();
    }
  }
  ...
  Future<bool> serialConnect() async {
    ...
    if (!serial.isRunning) {
      if (_comSelection != null) {
        serial.commPeriod = configData.commPeriod;
        await serial.openPort(_comSelection!, configData.baudRate, 0, 8, 1);
  ...
  set mode(int value) {
    SerialParse.setCommandMode(serial, value, SerialParse.txPacketMin);
    serial.sendPacket();
  }
  ...
  Future<int> getNumParameters() async {
    ...
    if (serial.isRunning) {
      ...
      SerialParse.setCommandMode(
          serial, SerialParse.readParameters, SerialParse.txPacketMin);
      ...
      SerialParse.setCommandMode(
          serial, SerialParse.nullMode, SerialParse.txPacketMin);
      serial.sendPacket();
  ...
  Future<bool> getParameters() async {
    ...
    if (serial.isRunning) {
      ...
      int parametersPerRx = SerialParse.numStatesPerPacket(serial) - 1;
  ...
  Future<bool> sendParameters() async {
    ...
    if (serial.isRunning) {
      _haltTelemetry = true;
      final parameters = configData.parameter;
      int statesPerTx = SerialParse.numStatesPerPacket(serial);
      int parametersPerTx = statesPerTx - 1;
      int bytesPerTx = SerialParse.requiredBytes + (statesPerTx * 4);
      int totalTransfers = (parameters.length / parametersPerTx).ceil();

      for (int transfer = 0; transfer < totalTransfers; transfer++) {
        SerialParse.setCommandMode(
            serial, SerialParse.writeParameters, bytesPerTx);
        SerialParse.setData32(
            serial, transfer, SerialParse.parameterTableIndex);
  ...
  Future<bool> flashParameters() async {
    ...
    if (serial.isRunning) {
      ...
      if (nullComplete) {
        SerialParse.setCommandMode(
            serial, SerialParse.flashParameters, SerialParse.txPacketMin);
        serial.sendPacket();
        serial.startWatchdog(parameterTimeout);
  ...
  Future<bool> initBootloader() async {
    ...
    if (serial.isRunning) {
      _haltTelemetry = true;
      SerialParse.setCommandMode(serial, SerialParse.nullMode, SerialParse.txPacketMin);
      serial.sendPacket();
      serial.startWatchdog(parameterTimeout);
      ...
      if (nullComplete) {
        SerialParse.setCommandMode(serial, SerialParse.reprogramBootMode, SerialParse.txPacketMin);
        serial.sendPacket();
        serial.startWatchdog(parameterTimeout);
  ...
  Future<void> openConfigFile(void Function() onComplete) async {
    ...
    receivePort.listen((message) {
      try {
        ...
        commPeriod = configData.commPeriod;
        setNetworkId(configData.networkId);
